#!/bin/bash
PIDFILE="/var/www/vizplus/info.viz.plus/module/ops_processing.pid"
if test -f "$PIDFILE"; then
	PID=`cat $PIDFILE`
	echo "PIDFILE exist $PID"
	PIDSTATUS=`ps -aux | grep [o]ps_processing.php | awk '{print $2}' | grep -v '^${PID}$'`
	echo "======="
...
php /var/www/vizplus/info.viz.plus/module/ops_processing.php > ~/logs/ops_processing.log &