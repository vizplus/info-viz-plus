#!/bin/bash
PIDFILE="/var/www/vizplus/info.viz.plus/module/props_snapshot.pid"
if test -f "$PIDFILE"; then
	PID=`cat $PIDFILE`
	echo "PIDFILE exist $PID"
	PIDSTATUS=`ps -aux | grep [p]rops_snapshot.php | awk '{print $2}' | grep -v '^${PID}$'`
	echo "======="
...
php /var/www/vizplus/info.viz.plus/module/props_snapshot.php > ~/logs/props_snapshot.log &