#!/bin/bash
PIDFILE="/var/www/vizplus/info.viz.plus/module/waterfall_irreversible.pid"
if test -f "$PIDFILE"; then
	PID=`cat $PIDFILE`
	echo "PIDFILE exist $PID"
	PIDSTATUS=`ps -aux | grep [w]aterfall_irreversible.php | awk '{print $2}' | grep -v '^${PID}$'`
...
php /var/www/vizplus/info.viz.plus/module/waterfall_irreversible.php > ~/logs/waterfall_irreversible.log &
