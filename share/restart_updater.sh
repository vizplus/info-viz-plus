#!/bin/bash
PIDFILE="/var/www/vizplus/info.viz.plus/module/updater.pid"
if test -f "$PIDFILE"; then
	PID=`cat $PIDFILE`
	echo "PIDFILE exist $PID"
	PIDSTATUS=`ps -aux | grep [u]pdater.php | awk '{print $2}' | grep -v '^${PID}$'`
	echo "======="
...
php /var/www/vizplus/info.viz.plus/module/updater.php > ~/logs/updater.log &