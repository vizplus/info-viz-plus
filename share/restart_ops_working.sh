#!/bin/bash
PIDFILE="/var/www/vizplus/info.viz.plus/module/ops_working.pid"
if test -f "$PIDFILE"; then
	PID=`cat $PIDFILE`
	echo "PIDFILE exist $PID"
	PIDSTATUS=`ps -aux | grep [o]ps_working.php | awk '{print $2}' | grep -v '^${PID}$'`
	echo "======="
...
php /var/www/vizplus/info.viz.plus/module/ops_working.php > ~/logs/ops_working.log &